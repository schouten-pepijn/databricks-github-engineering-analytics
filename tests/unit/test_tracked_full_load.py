...
def test_run_tracked_full_load_records_silver_failure_and_reraises(
    ...
    users_silver_load.assert_not_called()
...
def test_run_tracked_full_load_records_users_silver_failure_and_reraises(
    ...
    """Record failure when Users Silver fails after Issues Silver succeeds."""
    spark = Mock()
    repository = Mock()
    result = BronzeIngestionResult(records_extracted=3, batches_written=1)
    started_at = datetime(2026, 9, 20, 12, 0, tzinfo=UTC)
    finished_at = datetime(2026, 9, 20, 12, 5, tzinfo=UTC)
    failure = RuntimeError("Users Silver merge failed")
    expected_run_id = "12345678123456781234567812345678"
    ...
    with pytest.raises(RuntimeError, match="Users Silver merge failed") as exc_info:
        run_tracked_full_load(
            spark=spark,
            catalog="test_catalog",
            owner="psf",
    ...
    assert exc_info.value is failure
    issues_silver_load.assert_called_once_with(
        spark=spark,
        catalog="test_catalog",
        bronze_run_id=expected_run_id,
    )
    users_silver_load.assert_called_once_with(
        spark=spark,
        catalog="test_catalog",
        bronze_run_id=expected_run_id,
    )
    ...
    assert failed_run.status is PipelineRunStatus.FAILED
    assert failed_run.finished_at == finished_at
    assert failed_run.error_message == "Users Silver merge failed"
...
def test_run_tracked_full_load_records_failure_and_reraises(
    ...
    assert exc_info.value is failure
    issues_silver_load.assert_not_called()
    users_silver_load.assert_not_called()
